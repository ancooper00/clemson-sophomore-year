...
public class QueueApp {
     ...
     * @param queue is the queue to be checked
     * @return truth value associated with whether the queue is empty
     *
     * @pre NONE
     * @post checkEmpty = true iff queue.length() returns 0 AND
     *       does not change the queue
     */
    public static boolean checkEmpty(IQueue queue) {
        if (queue.length() == 0) {
     ...
     *
     * @pre NONE
     * @post checkFull = true iff queue.length() returns MAX_LENGTH AND
     *       does not change the queue
     */
    public static boolean checkFull(IQueue queue){
        if (queue.length() == IQueue.MAX_LENGTH){
     ...
     * @param q is the queue to be inserted into
     * @return valid position chosen by the user
     *
     * @pre q.length() < MAX_LENGTH
     * @post 0 < getPosInsert <= q.length()
     */
    public static int getPosInsert(IQueue q) {
        int p;
        System.out.println("What position to insert in?");
        Scanner inp = new Scanner(System.in);
        p = inp.nextInt();
        while ((p <= 0) || (p > q.length() + 1)) {
     ...
     * @param q is the queue the user wants to access
     * @return valid position chosen by the user
     *
     * @pre 0 < q.length() <= MAX_LENGTH
     * @post 0 < getPosGet <= q.length()
     */
    public static int getPosGet(IQueue q) {
        int p;
        System.out.println("What position to get from the Queue?");
        Scanner inp = new Scanner(System.in);
        p = inp.nextInt();
        while ((p <= 0) || (p > q.length())) {
            System.out.println("Not a valid position in the Queue!");
            System.out.println("What position to get from the Queue?");
     ...
     * @param q is the queue to be removed from
     * @return valid position chosen by the user
     *
     * @pre 0 < q.length() <= MAX_LENGTH
     * @post 0 < getPosInsert <= q.length()
     */
    public static int getPosRemove(IQueue q) {
        int p;
