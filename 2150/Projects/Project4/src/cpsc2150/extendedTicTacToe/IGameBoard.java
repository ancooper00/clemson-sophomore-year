...
public interface IGameBoard {
     ...
     *
     * @pre NONE
     * @post checkSpace = true iff  0 <= pos.Row < NUM_ROWS AND 0 <= pos.Column < NUM_COLS AND board[pos.Column][pos.Row] = " "
    ...
    /**
     * placeMarker places the player token (chosen by player) on the board at the position specified by marker
     * @param marker contains the location on the board where player wishes to place token
     * @param player is the token to be placed on the board
     *
     ...
     * @post checkForDraw = true if for all 0 <= i < NUM_ROWS and all 0 <= j < NUM_COLS, board[i][j] != " "
     */
     ...
     * @param player the token just played
     * @return the truth value describing whether a game has been won horizontally
     *
     * @pre 0 <= lastPos.Row < NUM_ROWS and 0 <= lastPos.Column < NUM_COLS
     ...
     *
     * @pre  0 <= pos.Row < NUM_ROWS AND 0 <= pos.Column < NUM_COLS
     *       [pos is located on the game board]
     * @post whatsAtPos = char at pos iff pos is located there, if not watsAtPos = ' '
     */
